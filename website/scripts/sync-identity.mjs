...
async function syncIdentity() {
  ...
  try {
    const response = await fetch(IDENTITY_URL, {
      ...
      headers: {
        'User-Agent': 'ask-widget-build-sync',
    ...
    if (!response.ok) {
      console.warn(
        `⚠️ Remote returned HTTP ${response.status}. Retaining local identity fallback.`,
      );
      return;
    }
    ...
    console.log('✅ Successfully synced identity.json to website/public/identity.json.');
  } catch (error) {
    clearTimeout(timeoutId);
